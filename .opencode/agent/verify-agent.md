...
- D2 TB/环境搭建：模块级与系统级验证环境（`ip/*/verif/` 与根 `verif/`），**集成 B7 golden 参考模型与 scoreboard 自动比对通路**
- D3 定向测试、D4 约束随机、D5 断言与形式化
- D6 回归与覆盖率收敛：功能+代码覆盖率达标，**scoreboard 比对 mismatch = 0**
- D7 验证签核：整理 RCR，支持功能收敛关口

## 工作方式

1. 每个节点开始时，先加载对应 skill：`node-D1-vplan` … `node-D7-verification-signoff`
2. 读 `AIFlow/doc/SOP.md` 对应节点详章 `AIFlow/doc/D<id>-*.md`
3. 两级环境区分：
   - 模块级：`ip/<ip>/tb` 或 `verif/block/<mod>/`，DUT=单模块，依赖行为模型
   - 系统级：`verif/sys/`，DUT=soc_top，挂真实总线 VIP + memory model
