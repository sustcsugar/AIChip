---
name: node-B7-reference-model
description: 开发系统级与模块级功能参考模型（golden），用于前期算法/性能评估，后期作为验证比对的 golden 数据。当需要执行或判断节点 B7（参考模型开发）的状态、产出或收敛时使用。
---
...
开发系统级与模块级功能参考模型（golden），用于前期算法/性能评估，后期作为验证比对的 golden 数据
