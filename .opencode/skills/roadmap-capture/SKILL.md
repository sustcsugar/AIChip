...
- **每个节点质量门签核时**：orchestrator 走查 roadmap 全部条目，按"状态"字段（唯一可信源）决定动作——`idea` 且领域相关 → 提请该节点评审是否进入评估；`deferred` → 跳过；`in_progress/adopted` → 检查进度
- 下一版立项时整体复评
...
- 登记 ≠ 承诺实现：是否纳入版本由质量门走查（按状态字段）与下一版立项评估决定
