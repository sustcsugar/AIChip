---
name: node-H5-phys-verify
description: DRC/LVS/ERC 物理验证（可选扩展）。当需要执行或判断节点 H5（物理验证）的状态、产出或收敛时使用。
---

# Node H5: 物理验证

> 本 skill 承载节点 H5 的执行工作流。节点完整定义（含人机职责）见 `AIFlow/doc/阶段H-物理设计/H5-物理验证.md`。
> 归属 agent：signoff-agent。执行前必须先读 `AIFlow/state/state-tracker.md` 确认节点处于 in_progress 且前置输入完整。

## 1. 目的

DRC/LVS/ERC 物理验证（可选扩展）
...
- 校验脚本：`python AIFlow/scripts/check_tracker.py --node H5`
...
- 节点详章：`AIFlow/doc/阶段H-物理设计/H5-物理验证.md`
