...
> 本 skill 承载节点 F2 的执行工作流。节点完整定义（含人机职责）见 `doc/阶段F-STA时序收敛/F2-STA分析/F2-STA分析.md`。
...
- 校验脚本：`python scripts/check_tracker.py --node F2`
...
- 节点详章：`doc/阶段F-STA时序收敛/F2-STA分析/F2-STA分析.md`
