---
name: node-C3-rtl-coding
description: 按微架构规格编写可综合 RTL，遵循编码规范。当需要执行或判断节点 C3（RTL编码）的状态、产出或收敛时使用。
---

# Node C3: RTL编码

> 本 skill 承载节点 C3 的执行工作流。节点完整定义（含人机职责）见 `AIFlow/doc/阶段C-微架构与RTL/C3-RTL编码.md`。
> 归属 agent：rtl-agent。执行前必须先读 `AIFlow/state/state-tracker.md` 确认节点处于 in_progress 且前置输入完整。

## 1. 目的

按微架构规格编写可综合 RTL，遵循编码规范
...
2. 若本 skill 的 `assets/templates/` 存在模板，先复制为工作文件
   - 本节点模板：`assets/templates/c3-selfcheck.md`（每模块自检留档）
...
- 校验脚本：`python AIFlow/scripts/check_tracker.py --node C3`
...
- 节点详章：`AIFlow/doc/阶段C-微架构与RTL/C3-RTL编码.md`
