...
> 本 skill 承载节点 {{ID}} 的执行工作流。节点完整定义（含人机职责）见 `doc/{{DOC}}`。
> 归属 agent：{{AGENT}}。执行前必须先读 `state/state-tracker.md` 确认节点处于 in_progress 且前置输入完整。
...
- 校验脚本：`python scripts/check_tracker.py --node {{ID}}`
