...
> 应用于所有 `gate: 评审` / `gate: 人工签字` 的节点。评审结果必须记录到 `AIFlow/state/state-decisions.md`。

## 触发时机

- 节点执行完成、收敛判据自检通过，orchestrator 将节点置为 `waiting_review`
- **硬前置：`governance-retro` 治理回顾已完成**（三层审核 + 机器三查），回顾结论须已记入评审材料；无回顾不进门
- 人工要求对某节点/产物评审
...
1. **准备评审材料**
   - 节点产出物清单（路径 + 摘要）
   - 收敛判据自检结果（对照 `AIFlow/doc/辅助文档/90-收敛判据速查表.md`）
   - 已知风险与遗留问题清单
...
4. **记录**
   ...
   - **规范回写**：评审意见若涉及规范/格式/命名（非仅本节点产物），必须同步回写节点详章、对应 skill 的 `assets/templates/`、`.opencode/skills/_shared/templates/`、`node-template`（如需），并由 orchestrator 加载 `skill-scaffold` skill、运行 `python .opencode/skills/skill-scaffold/scripts/scaffold_skills.py` 重新生成节点 skill（改 `nodes.json`/骨架模板后必须执行），确保 SOP 定义层与实际执行一致
