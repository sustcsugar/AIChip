---
name: node-{{ID}}-{{SLUG}}
description: {{DESCRIPTION}}。当需要执行或判断节点 {{ID}}（{{NAME}}）的状态、产出或收敛时使用。
---

# Node {{ID}}: {{NAME}}

> 本 skill 承载节点 {{ID}} 的执行工作流。节点完整定义（含人机职责）见 `AIFlow/doc/{{DOC}}`。
> 归属 agent：{{AGENT}}。执行前必须先读 `AIFlow/state/state-tracker.md` 确认节点处于 in_progress 且前置输入完整。
...
- 校验脚本：`python AIFlow/scripts/check_tracker.py --node {{ID}}`
