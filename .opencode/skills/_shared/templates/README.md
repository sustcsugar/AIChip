...
> 本目录是**跨节点共享模板**的唯一事实源（见 `AIFlow/state/state-decisions.md` ADR-011）。
...
| `adr-template.md` | 关键决策 ADR 条目格式 | `AIFlow/state/state-decisions.md`（orchestrator 登记） |
...
- 运行时产物（如 `ip_manifest.json`）保持在工作区，本目录只放模板示例
