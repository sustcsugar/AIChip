---
name: node-G1-packaging
description: 打包全部交付物：RTL/netlist/SDC/文档/报告。当需要执行或判断节点 G1（交付物打包）的状态、产出或收敛时使用。
---

# Node G1: 交付物打包

> 本 skill 承载节点 G1 的执行工作流。节点完整定义（含人机职责）见 `AIFlow/doc/阶段G-签核与交付/G1-交付物打包.md`。
> 归属 agent：signoff-agent。执行前必须先读 `AIFlow/state/state-tracker.md` 确认节点处于 in_progress 且前置输入完整。

## 1. 目的

打包全部交付物：RTL/netlist/SDC/文档/报告
...
- 校验脚本：`python AIFlow/scripts/check_tracker.py --node G1`
...
- 节点详章：`AIFlow/doc/阶段G-签核与交付/G1-交付物打包.md`
