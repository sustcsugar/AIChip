...
4. **产物配对一致性**：本节点若有多文件产物（主文档 + CSV/脚本/图等配套），核对配对组内口径一致（关键词抽查：协议名、编号、计数）；配对关系以文档登记表的配套列（若项目启用）或产物清单为准
...
| agent | agent 定义是否漂移/缺失纪律？ | 术语过时、路径失效、职责与实跑不符、**转述输入未回读原文核验** |
| skill | skill 是否有缺口/违规/漂移？ | 边界违规、手工定制未登记、模板缺失、执行卡点无指引 |

**审核附加纪律**：
- **自验范围声明**：agent 报告中的"校验通过/自验 PASS"必须附"检查了哪些文件"的显式清单；未声明范围的自验结论视为无效证据，orchestrator 退回补验
- **变更影响面 = 产物配对组**：任何勘误/修订的影响面按"主文档 + 配套产物"整组计算，禁止只改主文档；任务书派发勘误时应给出配对文件清单（或要求 agent 先盘点配对组）
