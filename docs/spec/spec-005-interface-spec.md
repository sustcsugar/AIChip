...
> 上位文档：`spec-001-PRD.md`（需求，20 REQ）、`spec-002-use-cases.md`（场景/用例，12 SC / 32 UC）、`spec-004-system-spec.md`（系统规格：BLOCK-01~14 / FS-001~020 / PPAC-001~020）
> 本文档定义"SoC 对外长什么样"：四类接口（引脚 / 总线 / 中断 / 存储映射）+ 时钟/复位 + 寄存器接口初版。
> **数据源声明（单一事实源）**：`spec-005-pins.csv` / `spec-005-memory-map.csv` / `spec-005-irq.csv` 为机器可读单一数据源；本文档表格由数据源生成（人工同步），冲突/遗漏检查以脚本 `AIFlow/scripts/a3_check_interface.py` 输出为准。
...
- 接口数据以 CSV 为单一数据源，供 C0 合同校验（`AIFlow/scripts/contract_check.py`）、B2 地址映射复用。
...
- 物理模块引用一律 `BLOCK-NN`（BLOCK-01~14，见 spec-004 §2.1）；需求引用 `REQ-NNN`；功能规格 `FS-NNN`；指标 `PPAC-NNN`；用例 `UC-<功能域>-NNN`。
- 未决项 OI 格式 `OI-<节点ID>-<全局序号>`（本节点新增 OI-A3-006，序号全局递增接续 A2 之后，登记于 `spec-003-open-issues.md`）。
- 指标数字均为 spec-004 §3 PPAC 表引用，不另立数值。
...
| 寄存器接口初版 | 8 外设寄存器组（待 C1/C2 细化） | 本文档 §6 |
...
| 5 | AXI-M-BRIDGE | AMBA AXI4（A3 裁定统一 AXI4） | BLOCK-02（主）→ AXI 从设备 | 32 / 32 | 1（单 ID，桥后无并发主；突发/outstanding 深度 B3 细化） | 桥 AXI 主端口；PPAC-014 单次读时延 ≤ 10 周期 |
| 6 | AXI-S-SRAM | AXI4（从） | BLOCK-14 | 32 / 32 | 1 | AXI_SRAM 从端口（MEM-03）；固件加载目标 |
| 7 | AXI-S-PERIPH | AXI4（从） | BLOCK-03/05/06/07/08/09/10/11（8 个从端口） | 32 / 32 | 1 | 每外设 4KB 译码窗口（MEM-05~12）；寄存器访问 |

### 2.3 带宽需求（对照 M 指标）

| 接口 | 指标引用 | 需求 | 判定 |
|------|---------|------|------|
| AXI 总线（桥主） | PPAC-014 | 单次读时延 ≤ 10 系统时钟周期；峰值 1 传输/周期 @50MHz（200 MB/s 读或写） | 外设寄存器访问远低于峰值，无瓶颈 |
| SPI 通道（SPI master ↔ 外部从设备/Flash） | PPAC-006, PPAC-007 | SCLK ≥ 12.5 MHz（Fsys/4 分频配置）；1KB 连续传输零错误 | SPI FIFO 填充带宽 12.5 Mbps ≪ AXI 能力 ✓ |
| UART 通道 | PPAC-004, PPAC-005 | 9600–921600 bps 可配；验收 115200 bps 双向连续 100KB 零丢失 | 中断/轮询双模式 + 收发 FIFO（初版 ≥16B，C1 细化）|
| IIC 通道 | PPAC-008 | 标准 100 kbps / 快速 400 kbps | 低速外设，无 AXI 压力 ✓ |
| 固件启动加载（SPI Flash → AXI_SRAM） | PPAC-013 | 复位释放 → 固件首指令 ≤ 100 ms（Flash 模型加载 64KB 镜像） | 单线 SPI 12.5MHz 70% 评估 ≈ 60 ms < 100 ms ✓（OI-A3-006 已关闭） |
...
| GPIO | gpio[15:0] | inout | 16 | BLOCK-08（REQ-019/012, FS-012/017, PPAC-020） |
...
|------|------|------|---------|------|-----------|--------|--------|---------|---------|-------|------|
| clk_i | in | 1 | LVCMOS | — | — | 0 | POR | clk_i | — | BLOCK-11 | 外部时钟输入 10–50MHz，片上时钟管理生成 Fsys（FS-010, PPAC-002） |
...
| gpio[9] | inout | 1 | LVCMOS | 可配 2/4/8mA | 上拉 | input | POR | gpio[9] | pwm_out[0] | BLOCK-08 | AF0=GPIO9, AF1=PWM0（PPAC-009/010） |
...
| 5 | TIMER0 | BLOCK-10 | level | high | 可屏蔽 | 5 | 定时器周期中断（FS-009, PPAC-012，周期误差 ±1 时钟） | UC-INT-002 |
...
- 中断源合计 **10 路**：外部（GPIO_EXT）4 + 定时器 1 + 软件 1 + 外设 4 → 满足 **PPAC-019（≥6：外部 ≥4 + 定时器 1 + 软件 1）**。
...
| 时钟 | 频率 | 来源 | 域 | 说明 |
|------|------|------|-----|------|
| Fsys（系统时钟） | 50 MHz（10–50 可配） | clk_i 经 BLOCK-11 时钟管理分频生成 | 单系统时钟域（同步设计） | FS-010, PPAC-002；全系统唯一功能时钟域 |
| UART 波特率时钟 | 由 Fsys 分频 | BLOCK-03 | 同 Fsys 域 | PPAC-004：9600–921600 bps 可配 |
| SPI SCLK | ≤ Fsys/4（12.5 MHz） | BLOCK-05 分频 | 同 Fsys 域（输出到外部） | PPAC-006；SPI Flash 时钟由 SCLK 驱动（外部器件时钟） |
| IIC SCL | 100 / 400 kHz | BLOCK-06 分频 | 同 Fsys 域（开漏输出） | PPAC-008 |
| PWM 计数时钟 | Fsys 分频 | BLOCK-07 | 同 Fsys 域 | PPAC-009/010：1 kHz–1 MHz 输出 |
| JTAG TCK | 外部异步 | 外部输入 | 独立异步域 | 跨时钟域经同步器（C5 CDC 检查项，spec-004 §4.2） |

- **时钟使能（门控）**：BLOCK-11 提供每外设时钟使能（CLK_EN 寄存器），空闲模式门控降功耗（FS-013, PPAC-017）；唤醒源：外部中断 / GPIO 事件 / UART 事件（PPAC-018，唤醒时延 ≤ 10 μs）。
...
|------|--------|------|--------|--------------------------|
| 0x00 | CTRL | R/W | 0x0 | TX/RX 使能、波特率分频装载（FS-002, PPAC-004） |
...
| 0x1C | FIFO_CTRL | R/W | 0x0 | 收发 FIFO 深度/清空（初版 ≥16B，支撑 PPAC-005 连续 100KB 零丢失） |

### 6.2 SPI（BLOCK-05，基址 0x4000_1000）

| 偏移 | 寄存器 | 读写 | 复位值 | 字段要点（待 C1/C2 细化） |
|------|--------|------|--------|--------------------------|
| 0x00 | CTRL | R/W | 0x0 | master 使能、CPOL/CPHA（四模式，FS-005）、SCLK 分频（Fsys/4 上限，PPAC-006） |
| 0x04 | STATUS | R | 0x0 | BUSY / TX_EMPTY / RX_FULL |
| 0x08 | TX_DATA | W | — | 发送数据 |
| 0x0C | RX_DATA | R | 0x0 | 接收数据（1KB 零错误对拍，PPAC-007） |
...
| 偏移 | 寄存器 | 读写 | 复位值 | 字段要点（待 C1/C2 细化） |
|------|--------|------|--------|--------------------------|
| 0x00 | CTRL | R/W | 0x0 | 使能、模式（标准 100k / 快速 400k，PPAC-008） |
...
| 0x04 | PERIOD | R/W | 0x0 | 周期计数值（1 kHz–1 MHz，PPAC-010） |
| 0x08 | DUTY[0] | R/W | 0x0 | 通道 0 占空比（8 bit 分辨率 0–255，PPAC-009） |
...
| 0x04 | LOAD | R/W | 0x0 | 周期装载值（周期误差 ±1 时钟，PPAC-012） |
...
| 偏移 | 寄存器 | 读写 | 复位值 | 字段要点（待 C1/C2 细化） |
|------|--------|------|--------|--------------------------|
| 0x00 | CLK_DIV | R/W | 0x0 | Fsys 分频配置（10–50 MHz，PPAC-002） |
| 0x04 | CLK_EN | R/W | 0x0 | 每外设时钟使能/门控（FS-013, PPAC-017；空闲模式降功耗） |
...
| MEM-14 | — | SPI Flash（外部器件） | —（无地址映射） | — | 8 | PIO | BLOCK-13 | 固件非易失存储；经 SPI master（BLOCK-05）PIO 访问（FS-003/011, PPAC-013） |
| MEM-15 | AXI | 未映射区 | 0x5000_0000 | 0xB000_0000 | 32 | — | BLOCK-02 | 未映射，访问错误 |

> **地址重叠检查**：全部 14 个地址映射区（MEM-01~13, 15）经 `AIFlow/scripts/a3_check_interface.py` 扫描，基址+大小无重叠、不越 32-bit 空间、无重复译码（脚本输出见自检报告）；MEM-14（SPI Flash）为外部器件 PIO 访问，无地址映射，不参与译码。
...
> 下表为机器可读遗漏核对表（`AIFlow/scripts/a3_check_interface.py --spec` 校验全 32 UC 均已覆盖）。**覆盖全部 12 SC / 32 UC**。
...
|----|------|------|----------------------|------|
| OI-A3-006 | **SPI Flash 启动加载带宽 vs PPAC-013 启动时延矛盾**：PPAC-006 限 SCLK ≤ 12.5 MHz（Fsys/4），单线 SPI 读 64KB 镜像满速 ≈ 41.9 ms、70% 评估 ≈ 60 ms，原 PPAC-013 要求 ≤ 5 ms。 | PPAC-006/PPAC-013 指标冲突；BOOT 通路带宽需求（§2.3）；BLOCK-05/13 | 人工裁定：PPAC-013 放宽至 ≤ 100 ms（70% 评估 ≈ 60 ms 留裕量）；保持单线 SPI @ Fsys/4=12.5 MHz；高速 SPI（160 MHz 级 QSPI）登记为 B1 时钟架构评估项 | closed（2026-08-20，用户裁定） |
...
| 地址映射细化（Boot ROM 容量、AXI_SRAM 容量） | B2 地址映射 | 64KB SRAM 支撑 PPAC-013 镜像；Boot ROM 4KB 若超需调整 |
...
| 判据（DoD） | 度量 | 结论 |
|-------------|------|------|
| 四类接口清单齐备（引脚/总线/中断/存储映射）+ 时钟复位 + 寄存器初版 | 引脚 23 / 总线 7 / 中断 10 / 存储 15（映射 14 + 外部 1）/ 时钟复位 2 / 寄存器 8 组 | ✅ |
...
> 指标接口对应（PPAC 表唯一事实源）：PPAC-004/005（UART §3/§6.1）、PPAC-006/007（SPI §3/§6.2）、PPAC-008（IIC §3/§6.3）、PPAC-009/010（PWM §3/§6.4）、PPAC-011/019（中断 §4）、PPAC-012（定时器 §4/§6.7）、PPAC-013（启动通路 §2.3/§7.2，含 OI-A3-006）、PPAC-014（AXI §2.2）、PPAC-016/017/018（CLK_RST §5.1/§6.8）、PPAC-020（GPIO §3.1）。
...
| v0.1a | 2026-08-21 | 勘误（ADR-018）：外设寄存器组/从端口计数 9→8（§1.3 / §2.2 / §10，共 3 处；实际为 8 组：§6.1~6.8 / MEM-05~12）；a3_check_interface.py 新增 A3-D7 三方计数一致性校验 | spec-agent |