...
用法:
    python AIFlow/scripts/a2_check_ppac.py --spec docs/spec/spec-004-system-spec.md \
                                   --prd  docs/spec/spec-001-PRD.md

输出:
    性能指标条目数 + 四要素(目标值/单位/测试方法/来源需求)完整性检查
    功能规格条目数 + 来源 REQ/SC 完整性检查
    PRD 全部 REQ 的规格映射覆盖率检查(每 REQ 至少映射一条 FS/PPAC)
...
def main():
    ...
    ppac_block = section_until(spec, "## 3. 性能指标")
    ppac_rows = [r for r in table_rows(ppac_block) if r and M_RE.fullmatch(r[0])]
    ppac_missing = []
    for r in ppac_rows:
        # r = [id, name, target, unit, method, source]
        for i, label in ((2, "目标值"), (3, "单位"), (4, "测试方法"), (5, "来源需求")):
            if i >= len(r) or not r[i]:
                ppac_missing.append((r[0], label))
    ppac_no_req = [r[0] for r in ppac_rows if len(r) < 6 or not REQ_RE.search(r[5])]
    ppac_no_sc = [r[0] for r in ppac_rows if len(r) < 6 or not SC_RE.search(r[5])]
    # 测试方法归属检查（仿真/综合/功耗/STA 至少命中一类）
    ppac_no_method_tool = []
    for r in ppac_rows:
        if len(r) < 5 or not r[4]:
            ppac_no_method_tool.append(r[0])
            continue
    ...
    spec_all_text = " ".join(r for r in ppac_rows for r in r) + " " + " ".join(
    ...
    # ---- 5. 五类指标覆盖 ----
    categories = {"Fmax": ["PPAC-002", "PPAC-003"], "吞吐": ["PPAC-005", "PPAC-006", "PPAC-008"],
                  "面积": ["PPAC-015"], "功耗": ["PPAC-016", "PPAC-017"], "时延": ["PPAC-011", "PPAC-012", "PPAC-013", "PPAC-014", "PPAC-018"]}
    ppac_ids = {r[0] for r in ppac_rows}
    missing_cat = [c for c, ids in categories.items() if not (set(ids) & ppac_ids)]

    # ---- 输出 ----
    print("=" * 60)
    print("[A2 check_ppac] 系统规格完整性校验报告")
    print("=" * 60)
    print(f"PRD REQ 条目数: {len(prd_req_ids)}")
    print(f"性能指标条目数: {len(ppac_rows)}")
    print(f"功能规格条目数: {len(fs_rows)}")
    print("-" * 60)
    print("[指标四要素(目标值/单位/测试方法/来源需求)] "
          + ("OK, 全部完整" if not ppac_missing else "缺失: " + ", ".join(f"{a}.{b}" for a, b in ppac_missing)))
    print("[指标来源 REQ 标注] " + ("OK" if not ppac_no_req else "缺失: " + ", ".join(ppac_no_req)))
    print("[指标来源 SC 标注] " + ("OK" if not ppac_no_sc else "缺失: " + ", ".join(ppac_no_sc)))
    print("[指标测试方法工具归属] " + ("OK" if not ppac_no_method_tool else "未命中工具链: " + ", ".join(ppac_no_method_tool)))
    ...
    failed = bool(ppac_missing or ppac_no_req or ppac_no_sc or ppac_no_method_tool
                  or fs_missing or uncovered_req or missing_cat)
    print(f"  [A2-D1] 指标四要素完整           : {'满足' if not ppac_missing else '未满足'}")
    print(f"  [A2-D2] 指标关联 REQ/SC          : {'满足' if not (ppac_no_req or ppac_no_sc) else '未满足'}")
    print(f"  [A2-D3] 指标可测试(方法+工具链)   : {'满足' if not ppac_no_method_tool else '未满足'}")
