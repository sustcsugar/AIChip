...
### ADR-022 — A 阶段资产质量审查修复（P1/P2/P3 三项）
- 日期：2026-08-21
- 状态：已确认
- 背景：用户指令对 A 阶段 agent/skill + 跨切面 skill 做质量审查，发现 P1×2 / P2×3 / P1×1 项缺陷（评级：ip-discipline 最佳样例，spec-agent 与节点骨架需修）
- 决策与执行：
  1. **P1** spec-agent.md：编号语义 M→PPAC（含 REQ→FS→PPAC 主脉络、"PPAC 表唯一事实源"）、补 PAD/IRQ/MEM 编号、详章路径修正（`AIFlow/doc/阶段A-.../<ID>-<名称>.md`）
  2. **P2** 骨架模板：步骤编号连续化（1-8，消除重复）、C3 专属示例通用化（"若详章指明存在规范/标准文档"）、§5 DoD 改"一句话摘要 + 唯一事实源指向（详章 + 90 速查表）"、§7 输出产物同步 → **46 节点 skill 第二次全量再生成**，node-A1 OI 定制块回写
  3. **P3** convergence-judge：判据读取改为"唯一事实源 + 不凭记忆"，证据要求"文件路径 + 可复现命令，禁止转述性结论"，新增结构类证据（矩阵/孤儿/冲突扫描 + 退出码）
- 验证：workflow_audit Blocker=0；资产边界扫描（ADR-021 口径）清零；check_tracker 正常
- 后果：skill 信息密度与防错性提升；DoD 单一事实源收敛于详章+速查表，skill 只承载索引
- 依据：用户 2026-08-21 指示（三项都执行）

### ADR-023 — 节点治理回顾回路（governance-retro）
- 日期：2026-08-21
- 状态：已确认（用户指令即骨架变更授权）
- 背景：用户要求每个节点完成、质量门前加入治理层/agent/skill 三层审核，形成节点级回顾-评审-更新完整反馈回路，保证治理层正确性/纯洁性/专业性，加速临时资产沉淀为固定资产
- 决策：
  1. **回路位置**：convergence-judge 判定 converged 后、置 waiting_review 前，治理回顾为**质量门硬前置**（无回顾不进门）
  2. **责任分配**：领域 agent=反馈源（强制输出 friction log）；**orchestrator=回路负责人**（机器三查+三层审核+处置+资产沉淀评估，治理层/skill/agent 唯一修改者）；人类=高影响变更裁定者（骨架/SOP 流程模型/agent 权限边界）
  3. **处置三出口**：当场修 / 登记 ADR / 登记 RMP（idea，进质量门走查）
  4. **资产沉淀阀门**：临时资产逐项判定——通用可复用→内化进 skill assets/ 或 _shared/templates/；项目专属→留项目区；一次性→丢弃留痕
- 执行：新建跨切面 skill `governance-retro`（orchestrator 专用）；orchestrator.md 工作流插入步骤；review-gate 触发条件加硬前置；骨架模板 Measure 加 friction log 输出（步骤增至 1-9）→ 46 节点 skill 第三次全量再生成；node-A1 OI 定制块回写
- 后果：每节点形成"执行→判据→治理回顾→质量门"完整闭环；治理缺陷发现时点从"阶段末人工审查"提前到"节点即时回顾"；骨架再生成频度上升，定制内容必须走 nodes.json/KNOWN_CUSTOM 正规化
- 依据：用户 2026-08-21 指示

### ADR-024 — arch-008 AXI4 术语勘误 + 派发输入纪律加固
- 日期：2026-08-21
- 状态：已确认
- 背景：B1 质量门审查发现 arch-008 通篇误写 AXI4-Lite（9 处）。根因：orchestrator 派发任务书把 ADR-009 裁定"统一 AMBA AXI4（全量）"错误转述为"AXI4-Lite 统一（B3 可评估升级）"——将评审前候选口径当成裁定后结论；arch-agent 忠实执行了错误输入
- 决策：
  1. 勘误：arch-008 九处 AXI4-Lite→AXI4，R7 风险改写为"突发/outstanding/ID 宽度 B3 量化"，B3 交接项同步；冻结基线 spec-v1.0 不受影响（其本身正确）
  2. **派发输入纪律加固**：orchestrator 派发任务涉及"已裁定决策"时，必须在任务书中给出 state-decisions.md 的 ADR 编号+原文引用（或要求 agent 回读原文核对），禁止凭记忆转述结论；agent 收到"已裁定决策"时应回读原文核验，发现不一致即上报而非执行
  3. 由此衍生的架构问题（AXI4 全量下外设是否加 AXI4→AXI4-Lite 二级桥接）已在 arch-008 §10 补分析，列为 B1 质量门新裁定项（结构 α/β）
- 依据：用户 2026-08-21 质量门质询（"是 RIB-AXI4 桥还是 RIB-AXI_lite 桥"）

### ADR-025 — B1 质量门裁定：架构方案 A + 从侧结构 β；RMP-003/004 采纳落地
- 日期：2026-08-21
- 状态：已确认（用户裁定）
- 决策：
  1. **架构方案 A（单主分区桥接式）采纳**，B/C 否决
  2. **从侧结构 β 采纳**（α 否决）：RIB↔AXI4 全量桥（ADR-009）→ AXI_IB 从侧分层——AXI_SRAM 走 AXI4 全量（保留突发），8 寄存器外设经 AXI4→AXI4-Lite 转换器（归 AXI_IB glue，归属随 R6 待 B3）降 Lite；寄存器区突发访问处理策略待 B3/C2
  3. **RMP-003 采纳并落地**：全部校验/脚手架脚本输出统一 UTF-8（reconfigure），GBK 控制台冒烟 10/10 通过
  4. **RMP-004 采纳并落地**：governance-retro 机器三查增至四查（+产物配对一致性）+ 审核附加纪律（自验范围声明；变更影响面=产物配对组）+ 骨架 Judge 强化 → 46 skill 第四次全量再生成
...
| 2026-08-21 | B1 系统架构 | 通过 | 用户 | 质量门质询发现 AXI4-Lite 误写（ADR-024 勘误）后裁定：方案 A + 从侧结构 β（ADR-025）；RMP-003/004 采纳落地；arch-008 v1.2 签字（B5 整体架构冻结） |
