...
- [ ] 前序节点产物（由 `AIFlow/state/state-tracker.md` 确认 passed）：E3 逻辑综合（检查通过）、E1 约束开发、E2 库环境
- [ ] E3 门级网表 `soc/syn/output/top_netlist.v` 与综合数据库
...
- 自定义脚本：`AIFlow/scripts/parse_drc.py` 解析报告 → 生成结构化违例清单 `drc_violations.csv`
...
- `soc/syn/reports/drc_report.rpt` — `report_design_rules`/`report_constraint` DRC 原始报告
- `soc/syn/reports/drc_violations.csv` — 逐条违例清单（类型/网络/单元/数值/处理去向）
- `soc/syn/reports/drc_checklist.md` — DRC 检查结论（clean 状态或已知项登记）
