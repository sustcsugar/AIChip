...
- [ ] G3 基线冻结（release tag + 交付包，`AIFlow/state/state-tracker.md` 中 G3 = passed）
...
- `soc/pnr/H1_floorplan/`：`floorplan.tcl`、`macro_placement.tcl`、`power_grid.tcl`
- `soc/pnr/H1_floorplan/floorplan.def`（或 `.fp`）
- `soc/pnr/H1_floorplan/QoR_floorplan.rpt`（面积/利用率/congestion/IR-drop 预估）
- 宏摆放示意图与 congestion 热图（png/pdf）
- 更新 `AIFlow/state/state-tracker.md`：H1 = passed（orchestrator 写入）
