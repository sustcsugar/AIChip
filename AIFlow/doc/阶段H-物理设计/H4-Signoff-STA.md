...
- [ ] H3 CTS 通过（cts.def + CTS DRC clean，`AIFlow/state/state-tracker.md` 中 H3 = passed）
...
- `soc/pnr/H4_signoff_sta/`：`sta_signoff.tcl`、MMMC 场景文件
- `soc/pnr/H4_signoff_sta/signoff_spf.spef`（签核级寄生）
- `soc/pnr/H4_signoff_sta/signoff_sta_summary.rpt`（全角 WNS/TNS/违例路径）
- `soc/pnr/H4_signoff_sta/violation_detail.rpt`（违例根因，如有）
- 更新 `AIFlow/state/state-tracker.md`：H4 = passed（orchestrator 写入）
