...
- [ ] H1 floorplan 通过（`.def` + QoR 报告，`AIFlow/state/state-tracker.md` 中 H1 = passed）
...
- `soc/pnr/H2_place_route/`：`place_route.tcl`、`place.def`、`route.def`
- `soc/pnr/H2_place_route/route_drc.rpt`（DRC 违例清单，clean）
- `soc/pnr/H2_place_route/congestion.rpt`、`utilization.rpt`
- `soc/pnr/H2_place_route/prects_sta.rpt`（SPEF + WNS/TNS 基线）
- 更新 `AIFlow/state/state-tracker.md`：H2 = passed（orchestrator 写入）
