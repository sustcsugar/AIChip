...
- [ ] H2 Place & Route 通过（route.def + DRC clean，`AIFlow/state/state-tracker.md` 中 H2 = passed）
...
- `soc/pnr/H3_cts/`：`cts.tcl`、`cts_spec`、`cts.def`
- `soc/pnr/H3_cts/clock_qor.rpt`（skew/latency/transition）
- `soc/pnr/H3_cts/cts_drc.rpt`（时钟 DRC，clean）
- `soc/pnr/H3_cts/postcts_sta.rpt`（WNS/TNS + hold 检查）
- 更新 `AIFlow/state/state-tracker.md`：H3 = passed（orchestrator 写入）
