...
6. **寄存器模型**：以 C1 产出的 SystemRDL `.rdl` 为源（按 `AIFlow/state/state-decisions.md` 中工具链决策执行），`peakrdl-uvm` 生成 UVM RAL 模型（预测镜像/后门访问），挂入模块级 env；cocotb 场景用 `peakrdl-python` 生成的 Python 寄存器访问层替代。
