...
- 行为模型来源：`verif/common/`（共享行为模型/VIP）；复用 IP 可挂其 `ip/<ip>/model/` 行为模型。
...
- [ ] 行为模型/VIP 可用：`verif/common/`（或 IP `model/`）
...
- 运行示例：
  ```bash
  # 单模块 smoke（cocotb）
  cd verif/block/<mod> && make
  # 或显式
  python -m cocotb --hdl-verilator ... # 依 Makefile 封装
  ```
- 依赖：行为模型/VIP 从 `verif/common/` 引用（路径在 TB 内 `--include`）。
- 约束：block TB 的 DUT 只含 `<mod>` 顶层，**不得实例化其他设计模块**（违者 C6 判拒）。
- 报告：`verif/block/<mod>/results/smoke_report.md`。
