...
**目的**：依据 C1 微架构规格与 C2 模块接口契约，为每个自研模块编写**可综合**的 RTL，严格遵循编码规范（命名、位宽、同步设计、防 latch/多驱动、时钟域隔离、参数化）。模块级 RTL 写入 `rtl/<mod>/`（SoC 自研模块）或 `ip/<ip>/rtl/`（IP 项目内模块）。

**范围**：
- 覆盖全部自研模块编码；glue 逻辑（复位生成、时钟管理、中断仲裁、pinmux）在本节点一并编码（在 `rtl/` 下按模块目录组织）。
...
- [ ] 编码规范（唯一真相源）：`AIFlow/doc/阶段C-微架构与RTL/C3-编码规范.md` — 可综合子集、命名、位宽、同步设计、CDC、注释、违规分级
- [ ] C0 的 `ip_manifest.json`（filelist 生成依据，供编译脚本引用）
- [ ] 目标工作区已存在：`rtl/<mod>/`（SoC）、`ip/<ip>/rtl/`（IP）
...
- 编译 / lint-only：
  ```bash
  # 单模块语法检查
  verilator --lint-only -Wall --top-module <mod> rtl/<mod>/<mod>.sv

  # 系统级编译（引入 manifest IP 与 defines，C3 末期可先行验证可集成性）
  verilator --binary --top-module soc_top -f build/filelist.f
  ```
- filelist 生成：`python AIFlow/scripts/build_manifest.py --filelist > build/filelist.f`
- RTL 存放：
  - SoC 自研模块：`rtl/<mod>/<mod>.sv`
  - IP 项目内模块：`ip/<ip>/rtl/<mod>.sv`
  - 公共定义：`rtl/common/defines.svh`
- 参考：`AIFlow/doc/阶段C-微架构与RTL/C3-编码规范.md`（规范）、C1 微架构文档、C2 接口契约
...
5. 每模块有人工放行记录（`AIFlow/state/state-decisions.md` 或代码评审 comment）。
