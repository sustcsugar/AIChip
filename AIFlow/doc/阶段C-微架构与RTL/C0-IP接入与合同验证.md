...
**目的**：将 B6 集成规划中选定的复用 IP 正式接入 SoC。验证每个 IP 的接口合同（总线 AXI、寄存器映射 Regmap、复位、时钟、中断）与 SoC 集成规格逐项一致，并将 IP 版本与引用模式（`model` / `rtl`）固定（pin）到 `ip_manifest.json`，建立 SoC 只读消费基线。

**范围**：
- 覆盖 manifest 中全部复用 IP（如 mipi/usb/ddr/axi_uart）。
- 验证对象是**接口合同**（`ip/<ip>/AIFlow/doc/interface-contract.md`）与 SoC 集成规格的**一致性**，不验证 IP 内部功能正确性（属 IP 项目自身 D 阶段收敛）。
- 本节点不修改任何 `ip/<ip>/` 下的源码，遵守 ip-discipline 纪律。

## 2. 输入产物（前置条件）

- [ ] B6 集成规划（passed）：IP 选型表、版本基线、复用策略（`AIFlow/state/state-tracker.md` 确认）
...
python AIFlow/scripts/build_manifest.py --filelist

# 检查 tracker 前置条件
python AIFlow/scripts/check_tracker.py --summary
```

**manifest `mode` 切换规则**（`ip_manifest.json`）：
- `mode: "model"`：早期集成阶段，SoC 引用 `ip/<ip>/model/` 行为模型，仿真快、可用于 C6 模块级与早期系统 smoke。
- `mode: "rtl"`：IP 已 D7 签核发布 tag 后，切换为引用 `ip/<ip>/rtl/` 真实 RTL，供 C7 集成与 D 阶段全量回归。
...
4. `ip_manifest.json` 已更新且 `soc_version` 递增，git 已有提交记录（`git log` 可查）。
5. 无 ip-discipline 违规：比对过程中未发生任何对 `ip/<ip>/` 源码的写操作（git status 干净）。

## 7. 质量门与签字

- 质量门类型：**检查**（自动化比对 + orchestrator 收敛判据核验）
- 检查未通过不得进入 C1；`AIFlow/state/state-tracker.md` 中 C0 状态置 `passed` 后方可派发下一节点。
