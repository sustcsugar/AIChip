...
- [ ] `build/filelist.f`（由 `build_manifest.py --filelist` 生成，系统级 lint 输入）
- [ ] `rtl/common/defines.svh` 及参数/宏依赖就绪
...
- **系统级 lint**：`verilator --lint-only -Wall -f build/filelist.f`（含 manifest IP），检查模块间连接错误、未定义引用、位宽传递。
- 对每个 warning 归类并处置：
...
verilator --lint-only -Wall -f build/filelist.f

# 增强规则（可选，若已配置）
# verible-verilog-lint --rules=-common,style rtl/<mod>/<mod>.sv
