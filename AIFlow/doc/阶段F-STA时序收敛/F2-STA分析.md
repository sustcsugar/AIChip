...
- [ ] 前序节点产物（由 `AIFlow/state/state-tracker.md` 确认 passed）：F1 约束签核评审（人工签字）、E5 LEC、E4 DRC
- [ ] E3 门级网表 `top_netlist.v`；E2 全部 PVT/RC 角库
- [ ] F1 签核版 SDC `soc/sta/constraints/soc_top.sdc`
...
- 脚本与汇总：`AIFlow/scripts/run_sta.tcl`（遍历 scenario）、`AIFlow/scripts/parse_sta.py`（解析 → 汇总 WNS/TNS/违例表）
...
- `soc/sta/reports/sta_summary.csv` — 全 scenario WNS/TNS/违例数汇总
- `soc/sta/reports/sta_violations_max.rpt` / `sta_violations_min.rpt` — 全量违例路径
- `soc/sta/reports/sta_worst_path.rpt` — 各角最差路径
- `soc/sta/reports/sta_clock_quality.rpt` — 时钟 skew/transition 报告
- `soc/sta/reports/sta_log.txt` — 各 scenario 运行日志
- `soc/sta/reports/sta_scope.md` — scenario 矩阵与 WNS/TNS 口径定义记录
