...
- `work/soc/verif/common/`：VIP、memory model、行为模型、公共 sequence/utility。
- `work/soc/verif/block/<mod>/`：模块级 TB（tb_top、Makefile、文件清单、`tests/`）。
- `work/soc/verif/sys/`：系统级 TB（`soc_top` 挂载 AXI VIP + memory model）。
- 编译日志、smoke 运行日志、smoke 通过报告。
- `work/ip/<mod>/tb/` 下的 IP 自带 TB（如存在，纳入 block 级体系）。
