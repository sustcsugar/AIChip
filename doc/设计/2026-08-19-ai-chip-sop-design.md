...
- **IP 项目层**：mipi/usb/ddr/axi_uart 等独立收敛单元，自带 rtl/doc/tb/vip/model/constraint/RELEASE.md，收敛到 D7 签核即发布 tag。
...
| signoff-agent | F+G (F1–F5, G1–G3) | node-f1…f5, node-g1…g3 | STA 分析、违例修复协调、交付打包、双签核 | work/soc/sta, 交付包 |
...
配套 `scripts/check_tracker.py` 校验节点前后置条件。

## 9. 项目目录骨架

```
D:\work\AIChip\
├── opencode.json                     # 可选：default_agent=orchestrator 等
├── README.md                         # 实验说明
├── .opencode/
