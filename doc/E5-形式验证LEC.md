...
- Formality（`work/soc/syn/scripts/fm.tcl`）：
...
- 复验辅助：`scripts/check_lec.py` 解析报告 → 提取 not-equivalent/unmapped 计数与点位
...
- `work/soc/syn/reports/lec_report.log` — 验证结果原始报告
- `work/soc/syn/reports/lec_summary.md` — LEC 结论（等价状态、匹配统计、豁免清单）
- `work/soc/syn/reports/lec_unmatched.txt` / `lec_failing.txt` — 未匹配/未等价点位（如有，应为空或已豁免）
- `work/soc/syn/reports/lec_session` — Formality session（可复现性留档）
