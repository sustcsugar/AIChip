...
- [ ] 时钟/复位定义文件：`work/soc/docs/cdc/clk-def.json`（时钟域名称、频率、相位、复位域映射）
- [ ] IP 侧同步方案声明（来自 `work/ip/<ip>/doc/interface-contract.md`，C0 已验证）
...
- 开源主流程（本实验无商用 CDC 工具时的策略）：
  - 结构识别：基于 `clk-def.json` + 自研脚本 `scripts/cdc_check.py`——遍历 RTL 标注信号时钟域，输出跨域路径表。
...
- 配置与报告：
  - `work/soc/docs/cdc/clk-def.json`（时钟域定义）
  - `work/soc/docs/reports/c5-cdc.md`（跨域路径表 + 违例清单）
  - `work/soc/docs/reports/c5-waiver.md`（CDC waive 清单）
